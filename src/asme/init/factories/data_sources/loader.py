...
from asme.init.factories.data_sources.datasets.processor.last_item_mask import get_sequence_features
from data.collate import padded_session_collate, PadDirection, PadInformation
from data.datasets import ITEM_SEQ_ENTRY_NAME, POSITIVE_SAMPLES_ENTRY_NAME, NEGATIVE_SAMPLES_ENTRY_NAME, TARGET_SUFFIX
...
def _build_padding_info(context: Context,
    ...
    return {
        key: PadInformation(padding_token, max_seq_length, max_seq_step_length),
        key + TARGET_SUFFIX: PadInformation(padding_token, max_seq_length, max_seq_step_length)
    }
...
def _build_entries_to_pad(config: Config,
    ...
    for sequence_feature_info in sequence_features_info:
        padding_infos = _build_padding_info(context, sequence_feature_info)
