...
def build_tokenizer_providers_from_config(dataset_config: providers.ConfigurationOption) -> providers.Factory:
    return providers.Factory(
        build_tokenizers,
        dataset_config
    )
...
def build_tokenizers(tokenizer_config: Dict[str, Any]) -> Dict[str, Tokenizer]:
    tokenizers = {}
    vocabulary_serializer = CSVVocabularyReaderWriter(tokenizer_config['tokenizer']['vocabulary']['delimiter'])
    vocabulary = provide_vocabulary(vocabulary_serializer, tokenizer_config['tokenizer']['vocabulary']['file'])
    tokenizer = provide_tokenizer(vocabulary, tokenizer_config['tokenizer']['special_tokens'])
    tokenizers['tokenizer'] = tokenizer

    additional_tokenizers = "additional_tokenizers"
    if additional_tokenizers in tokenizer_config:
        for key, config in tokenizer_config[additional_tokenizers].items():
            vocabulary_serializer = CSVVocabularyReaderWriter(config['vocabulary']['delimiter'])
            vocabulary = provide_vocabulary(vocabulary_serializer, config['vocabulary']['file'])
            tokenizer = provide_tokenizer(vocabulary, config['special_tokens'])
            tokenizers[key] = tokenizer
    return tokenizers


def build_tokenizer_provider(config: providers.Configuration, vocab: providers.Configuration) -> providers.Factory:
    vocabulary_serializer = providers.Factory(CSVVocabularyReaderWriter,
                                              config.tokenizers.tokenizer.vocabulary.delimiter)
    vocabulary = providers.Factory(provide_vocabulary, vocabulary_serializer, vocab)
    return providers.Factory(provide_tokenizer, vocabulary, config.tokenizers.tokenizer.special_tokens)
...
def build_session_loader_provider_factory(model_config: providers.ConfigurationOption,
                                          dataset_config: providers.ConfigurationOption,
                                          tokenizers_provider: providers.Provider,
                                          processors_providers: providers.Provider) -> providers.Factory:
    ...
    return providers.Factory(
        ...
        model_config.additional_attributes,
        tokenizers_provider
    )


def build_nextitem_loader_provider_factory(model_config: providers.ConfigurationOption,
                                           dataset_config: providers.ConfigurationOption,
                                           tokenizers_provider: providers.Provider,
                                           processors_provider: providers.Provider
                                           ) -> providers.Factory:
    dataset = build_nextitem_dataset_provider_factory(tokenizers_provider, processors_provider, dataset_config)
    dataset_loader_config = dataset_config.loader
    pad_direction = providers.Factory(to_pad_direction, dataset_loader_config.pad_direction)
...
def build_posneg_loader_provider_factory(dataset_config: providers.ConfigurationOption,
    ...
    dataset = build_posnet_dataset_provider_factory(tokenizers_provider, processor_provider_provider, dataset_config)
    dataset_loader_config = dataset_config.loader
    pad_direction = providers.Factory(to_pad_direction, dataset_loader_config.pad_direction)
...
def build_session_dataset_provider_factory(processors_provider: providers.Provider,
                                           dataset_config: providers.ConfigurationOption
                                           ) -> providers.Factory:
...
def build_nextitem_dataset_provider_factory(tokenizers_provider: providers.Provider,
    ...
    if attributes != None:
        for attr in attributes:
            for key, value in attr.items():
                entries_to_pad[key] = [max_seq_length,value['vocab_size']]
...
def provide_session_loader(dataset: Dataset,
    ...
    return DataLoader(
        ...
        collate_fn=padded_session_collate(
            pad_token_id=tokenizers["tokenizer"].pad_token_id,
            entries_to_pad=_build_entries_to_pad(max_seq_length, attributes, max_seq_step_length),
