...
class ConfigTrainerBuilder:
    ...
    def get_checkpoint_callback(self, config: ConfigTree) -> ModelCheckpoint:
        ...
        parameters["filepath"] = config.get_string("filepath", None)
        parameters["save_top_k"] = config.get_int("save_top_k", 3)
        parameters["save_last"] = config.get_bool("save_last", True)
