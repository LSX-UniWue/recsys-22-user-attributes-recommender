...
def get_multiple_item_recommendation_samples():
    return [
        build_sample(torch.tensor([[0, 0, 0, 4, 5]]), torch.tensor([[3, 4]]), None, 2, torch.tensor(1. + 0.6309)),
        build_sample(torch.tensor([[0, 0, 4, 5, 0]]), torch.tensor([[3, 4]]), None, 2, torch.tensor(1.)),
...
class TestDCG:
    ...
    def test_single_item_recommendation(self, sample: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int, torch.Tensor]):
        ...
        instance = DiscountedCumulativeGain(k=k)
